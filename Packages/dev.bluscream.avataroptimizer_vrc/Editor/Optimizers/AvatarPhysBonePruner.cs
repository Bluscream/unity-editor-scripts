...
namespace Bluscream.VRCAvatarOptimizer
{
    ...
    {
        private static readonly BluLog Log = BluLog.Get("AvatarPhysBonePruner");
        ...
        public static int PrunePhysBones(GameObject avatarRoot, PlatformProfile profile, PhysBonePruningStrategy strategy, Action<string> progressCallback = null)
        {
            ...
            Log.Info($"Found {pbList.Count} PhysBone component(s) and {pbColliders.Count} PhysBone Collider(s) on '{avatarRoot.name}'.");
            ...
            if (pbList.Count > targetComponents)
            {
                ...
                Log.Info($"[Pass 1] PhysBones {pbList.Count} > limit {targetComponents}. Pruning {toRemove} component(s) ({strategy}).");
                ...
                if (strategy == PhysBonePruningStrategy.InteractiveChecklist && !Application.isBatchMode)
                {
                    ...
                    {
                        Log.Info("[Pass 1] Checklist cancelled — falling back to automatic deepest-first selection.");
                    }
                }
                ...
                {
                    if (pb == null) continue;
                    Log.Info($"[Pass 1] Removing PhysBone on '{pb.gameObject.name}'");
                    pbList.Remove(pb);
            ...
            {
                Log.Info($"[Pass 1] PhysBone component count OK: {pbList.Count} / {targetComponents} limit.");
            }
            ...
            if (maxTransforms < int.MaxValue)
            {
                ...
                {
                    Log.Info($"[Pass 2] PhysBone transforms {totalTransforms} > limit {maxTransforms}. Pruning components (deepest first) until within budget.");
                    ...
                    while (totalTransforms > maxTransforms && pbList.Count > 0)
                    {
                        ...
                        Log.Info($"[Pass 2] Removing PhysBone on '{pb.gameObject.name}' ({t} transforms)");
                        Undo.DestroyObjectImmediate(pb);
                        totalTransforms -= t;
                        removedCount++;
                    }
                ...
                {
                    Log.Info($"[Pass 2] PhysBone transform count OK: {totalTransforms} / {maxTransforms} limit.");
                }
            }
            ...
            if (pbColliders.Count > targetColliders)
            {
                ...
                Log.Info($"[Pass 3] Colliders {pbColliders.Count} > limit {targetColliders}. Removing {toRemove} collider(s) (least referenced first).");

                for (int i = 0; i < toRemove && i < removalOrder.Count; i++)
                {
                    Component col = removalOrder[i];
                    if (col == null) continue;
                    Log.Info($"[Pass 3] Removing collider on '{col.gameObject.name}' ({(refCounts.TryGetValue(col, out int n) ? n : 0)} reference(s))");
            ...
            {
                Log.Info($"[Pass 3] Colliders OK: {pbColliders.Count} / {targetColliders} limit.");
            }
            ...
            Log.Info($"[Pass 4] Collision checks: {totalCollisionChecks} / {maxChecks} limit across {pbList.Count} remaining PBs.");
                ...
                foreach (Component pb in sortedPBs)
                {
                    ...
                    try
                    {
                        ...
                        if (collidersProp != null && collidersProp.isArray && collidersProp.arraySize > 0)
                        {
                            ...
                            Log.Info($"[Pass 4] Cleared collider list on '{pb.gameObject.name}': saved {before} checks. Running total: {totalCollisionChecks}");
                    ...
                    {
                        Log.Warn($"Could not clear colliders on {pb.name}: {e.Message}");
                    }
                }

                Log.Info($"[Pass 4] After collider list trimming: collision checks = {totalCollisionChecks} / {maxChecks}");
            }

            Log.Info($"Pruned {removedCount} PhysBone components/colliders to meet target rank '{profile.Rank}'.");
            return removedCount;
        }
