...
{
    /// <summary>
    /// Represents a single property entry for serialized component properties.
    /// </summary>
    [System.Serializable]
    ...
    /// <summary>
    /// Top-level utility functions for Bluscream packages (Unity / System only).
    /// </summary>
    public static class Utils
    {
        public const string COMMON_VERSION = "1.2.1";

        /// <summary>
        /// Checks if the active Unity build target platform is Mobile (Android or iOS).
        /// </summary>
        ...
        /// <summary>
        /// Checks if the Backup System package is available in the current project.
        /// </summary>
        public static bool IsBackupSystemAvailable()
        {
            return ReflectionHelper.TryFindType("Bluscream.BackupSystem.BackupSystem", out _) ||
                   ReflectionHelper.TryFindType("Bluscream.BackupSystem", out _);
        }
        ...
        /// <summary>
        /// Serializes a component to a list of property entries.
        /// </summary>
        public static List<ComponentPropertyEntry> SerializeComponentToPropertyList(SerializedObject so)
        {
            var entries = new List<ComponentPropertyEntry>();
            if (so == null) return entries;
            try
            {
                ...
                while (prop.NextVisible(enterChildren))
                {
                    ...
                    {
                        case SerializedPropertyType.Integer: value = prop.intValue.ToString(); break;
                        case SerializedPropertyType.Boolean: value = prop.boolValue ? "true" : "false"; break;
                        case SerializedPropertyType.Float: value = prop.floatValue.ToString(); break;
                        case SerializedPropertyType.String: value = prop.stringValue; break;
                        case SerializedPropertyType.Color: Color c = prop.colorValue; value = $"{c.r},{c.g},{c.b},{c.a}"; break;
                        case SerializedPropertyType.Vector2: Vector2 v2 = prop.vector2Value; value = $"{v2.x},{v2.y}"; break;
                        case SerializedPropertyType.Vector3: Vector3 v3 = prop.vector3Value; value = $"{v3.x},{v3.y},{v3.z}"; break;
                        case SerializedPropertyType.Vector4: Vector4 v4 = prop.vector4Value; value = $"{v4.x},{v4.y},{v4.z},{v4.w}"; break;
                        case SerializedPropertyType.Quaternion: Quaternion q = prop.quaternionValue; value = $"{q.x},{q.y},{q.z},{q.w}"; break;
                        case SerializedPropertyType.Rect: Rect rect = prop.rectValue; value = $"{rect.x},{rect.y},{rect.width},{rect.height}"; break;
                        case SerializedPropertyType.Bounds: Bounds bounds = prop.boundsValue; value = $"{bounds.center.x},{bounds.center.y},{bounds.center.z},{bounds.size.x},{bounds.size.y},{bounds.size.z}"; break;
                        case SerializedPropertyType.ObjectReference:
                            if (prop.objectReferenceValue != null)
                            {
                                string path = AssetDatabase.GetAssetPath(prop.objectReferenceValue);
                                value = string.IsNullOrEmpty(path) ? prop.objectReferenceValue.GetInstanceID().ToString() : path;
                            }
                            else value = "null";
                            break;
                        case SerializedPropertyType.Enum:
                            value = (prop.enumNames != null && prop.enumNames.Length > prop.enumValueIndex && prop.enumValueIndex >= 0) ? prop.enumNames[prop.enumValueIndex] : prop.enumValueIndex.ToString();
                            break;
                        case SerializedPropertyType.ArraySize: case SerializedPropertyType.LayerMask: value = prop.intValue.ToString(); break;
                        default: canSerialize = false; break;
                    }
                    if (canSerialize && value != null)
                        entries.Add(new ComponentPropertyEntry(prop.name, value, typeName));
                }
            }
            catch { }
            return entries;
        ...
        /// <summary>
        /// Manually serializes a component to a JSON string.
        /// </summary>
            ...
            try
            {
                ...
                while (prop.NextVisible(enterChildren))
                {
                    ...
                    switch (prop.propertyType)
                    {
                        ...
                        case SerializedPropertyType.Boolean: value = prop.boolValue ? "true" : "false"; break;
                        case SerializedPropertyType.Float: value = prop.floatValue.ToString(); break;
                        case SerializedPropertyType.String: value = $"\"{EscapeJsonString(prop.stringValue)}\""; break;
                        default: break;
                    }
                    if (value != null)
                    {
        ...
        /// <summary>
        /// Checks if a GameObject is a prefab asset.
        /// </summary>
        ...
        /// <summary>
        /// Gets the prefab asset from a prefab instance.
        /// </summary>
        ...
        /// <summary>
        /// Gets all available shaders in project.
        /// </summary>
        public static List<Shader> GetAllShaders()
        {
            ...
            string[] commonBuiltInShaders = new string[]
            {
                ...
                "Unlit/Transparent", "Unlit/Transparent Cutout", "Legacy Shaders/Diffuse",
                "Legacy Shaders/Specular", "Legacy Shaders/Bumped Diffuse", "Legacy Shaders/Bumped Specular",
                ...
                "Mobile/Diffuse", "Mobile/Bumped Diffuse", "Mobile/Bumped Specular",
                "Mobile/Unlit (Supports Lightmap)", "Mobile/VertexLit", "Sprites/Default",
                "Sprites/Diffuse", "UI/Default", "UI/Unlit/Detail", "UI/Unlit/Text",
                "UI/Unlit/Transparent", "Particles/Standard Surface", "Particles/Standard Unlit",
                "Particles/Additive", "Particles/Additive (Soft)", "Particles/Alpha Blended",
                "Particles/Alpha Blended Premultiply", "Particles/Multiply", "Particles/Multiply (Double)",
                "Particles/VertexLit Blended", "Skybox/6 Sided", "Skybox/Cubemap", "Skybox/Procedural"
            };
        ...
        /// <summary>
        /// Gets all project shaders grouped by folder path.
        /// </summary>
