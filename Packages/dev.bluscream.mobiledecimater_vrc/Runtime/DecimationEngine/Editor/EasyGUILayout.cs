...
namespace UnityMeshDecimation.UI {
	public static class EasyGUILayout {
		...
		public static bool FilePathField(string title, ref string value, string extension, string btnName = "Select", string defaultPath = "", Object target = null) {
			...
			if(GUILayout.Button(btnName, GUILayout.Width(50))){
				...
				if (!string.IsNullOrEmpty(defaultPath)) {
					directory = System.IO.Path.GetDirectoryName(defaultPath);
					fileName = System.IO.Path.GetFileName(defaultPath);
					// Convert project relative directory to absolute path
					if (!string.IsNullOrEmpty(directory)) {
						directory = System.IO.Path.GetFullPath(directory);
					}
				}
				string absPath = EditorUtility.SaveFilePanel(title, directory, fileName, extension);
				if (!string.IsNullOrEmpty(absPath)) {
					newValue = FileUtil.GetProjectRelativePath(absPath);
				}
			}
